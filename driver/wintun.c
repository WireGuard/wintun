...
TunProcessReceiveData(_Inout_ TUN_CTX *Ctx)
{
    ...
    while (!KeReadStateEvent(&Ctx->Device.Disconnected))
    {
        ...
        PVOID virtualAddr = (UCHAR *)MmGetMdlVirtualAddress(Ctx->Device.Receive.Mdl) + (ULONG)(Packet->Data - (UCHAR *)Ring);
        MDL *Mdl = IoAllocateMdl(virtualAddr, PacketSize, FALSE, FALSE, NULL);
        if (!Mdl)
            goto skipNbl;
        IoBuildPartialMdl(
